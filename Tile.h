...
public:
  Tile() : type(TILE_VOID) { set_light_level(TILE_DEFAULT_LIGHT_LEVEL); }

  Tile(const tile_type t) : type(t) {
    set_light_level(TILE_DEFAULT_LIGHT_LEVEL);
  }
  ~Tile() {}

  const tile_type get_type() const { return type; }
  const vector<entity_id> &get_entities() const { return entities; }
  ...
  const int get_light_level() const { return light_level; }
  const int get_max_light_level() const { return max_light_level; }
  void set_type(const tile_type t) { type = t; }
  void add_entity(const entity_id id) { entities.push_back(id); }
  void increase_light_level() { increase_light_level_by(1); }
  void decrease_light_level() { decrease_light_level_by(1); }
