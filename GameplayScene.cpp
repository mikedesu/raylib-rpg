...
bool GameplayScene::handle_dungeon_move_pos(const entity_id id,
    ...
    if(!Vector2Equals(c_pos, t_pos)) {
        ...
        case TILE_FLOOR_DOWNSTAIRS: {
            ...
            if(r) {
                dungeon_events.push_back(DungeonEvent(id, EVENT_ENTITY_MOVE_SUCCESS, t_pos));
...
Vector2 GameplayScene::handle_dungeon_move_dir(const entity_id id, const Vector2 direction) {
...
void GameplayScene::increase_lighting_at(const Vector2 loc, const int light_level, const int dist) {
    ...
    for(int j = 0; j < dist; j++) {
        ...
        for(int i = 0; i < dist - j; i++) {
            ...
            if(x0 >= 0 && x0 < gridsize && y0 >= 0 && y0 < gridsize) {
                dungeon_floor.get_tile_by_col_row(x0, y0).increase_light_level_by(light_incr);
            }

            if(y0 != y1) {
                if(x0 >= 0 && x0 < gridsize && y1 >= 0 && y1 < gridsize) {
                    dungeon_floor.get_tile_by_col_row(x0, y1).increase_light_level_by(light_incr);
                }
            }
            if(x0 != x1) {
                if(x1 >= 0 && x1 < gridsize && y0 >= 0 && y0 < gridsize) {
                    dungeon_floor.get_tile_by_col_row(x1, y0).increase_light_level_by(light_incr);
                }
                if(y0 != y1) {
...
void GameplayScene::decrease_lighting_at(const Vector2 loc, const int light_level, const int dist) {
    if(loc.x < 0 || loc.y < 0 || loc.x >= dungeon_floor.get_gridsize() ||
       loc.y >= dungeon_floor.get_gridsize()) {
        return;
    }
    ...
    for(int j = 0; j < dist; j++) {
        ...
        for(int i = 0; i < dist - j; i++) {
            ...
            const int y1 = loc.y + i;
            if(x0 >= 0 && x0 < gridsize && y0 >= 0 && y0 < gridsize) {
                dungeon_floor.get_tile_by_col_row(x0, y0).decrease_light_level_by(light_incr);
            }

            if(y0 != y1) {
                if(x0 >= 0 && x0 < gridsize && y1 >= 0 && y1 < gridsize) {
                    dungeon_floor.get_tile_by_col_row(x0, y1).decrease_light_level_by(light_incr);
                }
            }
            if(x0 != x1) {
                if(x1 >= 0 && x1 < gridsize && y0 >= 0 && y0 < gridsize) {
                    dungeon_floor.get_tile_by_col_row(x1, y0).decrease_light_level_by(light_incr);
                }
                if(y0 != y1) {
                    if(x1 >= 0 && x1 < gridsize && y1 >= 0 && y1 < gridsize) {
...
bool GameplayScene::init() {
    if(!get_has_been_initialized()) {
        ...
        message_pane = make_shared<MessagePane>(get_global_font(),
                                                get_global_font().baseSize,
                                                14,
...
const entity_id GameplayScene::spawn_player(const Vector2 pos) {
    ...
    entity_id id = spawn_entity("player", 0, 0, SPRITETYPE_PLAYER, true, get_global_scale());
    player_id = id;
...
const entity_id GameplayScene::spawn_column(const Vector2 pos) {
    ...
    entity_id id = spawn_entity("column", 0, 0, SPRITETYPE_WALL, true, get_global_scale());
...
const entity_id GameplayScene::spawn_torch(const Vector2 pos) {
    ...
    entity_id id = spawn_entity("torch", 0, 0, SPRITETYPE_ITEM, true, get_global_scale());
...
inline void GameplayScene::draw_debug_panel() {
    ...
    DrawTextEx(get_global_font(), camera_info_str.c_str(), loc, fontsize, alpha, c1);
...
inline void GameplayScene::draw_controls() {
    ...
    DrawRectangle(x, y, w, h, c0);
    DrawRectangleLines(x, y, w, h, c2);
    DrawTextEx(get_global_font(),
               s.c_str(),
               (Vector2){(float)x + pad, (float)y + pad},
               fontsize,
               0.5f,
               c1);
}
...
inline void GameplayScene::draw_hud() {
    ...
    if(last_tile_click_pos.x >= 0 && last_tile_click_pos.y >= 0) {
        tile_type_str =
            dungeon_floor.get_tile_by_col_row(last_tile_click_pos.x, last_tile_click_pos.y)
                .get_type_str();
    ...
    s += "Position: " + to_string((int)dungeon_floor.get_entity_position(player_id).x) + ", " +
...
inline void GameplayScene::draw_tile(const string tile_key, const int i, const int j) {
...
inline void GameplayScene::handle_tile_click(const Rectangle dest, const int i, const int j) {
    if(tile_is_selected) {
...
inline void GameplayScene::handle_popup_manager() {
    if(show_test_popup) {
        if(get_popup_manager() != nullptr) {
            const Vector2 dest_vector = (Vector2){get_sprite(player_id)->get_dest().x - 20,
