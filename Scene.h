...
public:
    ...
    bool load_textures() {
        ...
        char *asset_name = nullptr, *num_frames = nullptr, *contexts = nullptr,
             *is_player = nullptr, *width = nullptr, *height = nullptr, *asset_path = nullptr;
        bool retval = false;
        if(texture_filepath == "") {
            ...
            if(file == NULL) {
                ...
                while(fgets(line, sizeof(line), file)) {
                    ...
                    bool result = load_texture(asset_name,
                                               asset_path,
                                               atoi(num_frames),
                                               atoi(contexts),
                                               atoi(is_player),
    ...
    bool load_texture(const char* asset_name,
    ...
    entity_id spawn_entity(const char* key,
                           const float x,
                           const float y,
        ...
        shared_ptr<Sprite> s = make_shared<Sprite>(textures[key]->texture,
                                                   textures[key]->num_frames,
                                                   textures[key]->contexts,
                                                   x,
                                                   y,
                                                   textures[key]->width,
                                                   textures[key]->height,
    ...
    entity_id spawn_entity(
        const char* key, const float x, const float y, const sprite_type type, const bool is_anim) {
    ...
    void set_camera_default_values() {
        camera2d.target.x = camera2d.target.y = camera2d.offset.x = camera2d.offset.y = 0;
    ...
    void set_scene_transition(SceneTransition st) {
        transition = st;
    }
